...
- [x] Ok, resources such as audio and video lack any context - most should have some information (like a date, description) that would be good to include beside beneath the link in the corresponding resources link oge (e.g. "Adio) At least in abridged form.   -- new shared partials/resource-list.html (same pattern as events-list.html) now shows date + description under each title on Audio/Video/Text/Language Study listings. Note: most of those dates are leftover WordPress-import timestamps, not real recording dates -- flagged this since dates were deliberately stripped from Resources elsewhere for that exact reason, you said include them anyway.
